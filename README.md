...
## Global services vs Dependency Injection (DI)

Lot of problems can come from using global objects, as loosing the ability of ES6 modules to do things like "three shaking" and other features and it is a bad practice. So the best solution for this kind of problem is Dependency Injection. So when we declare in the constructor of a class where it will be used it means that the class declared within the constructor has an instance of it created whenever it is referenced in a constructor function. It happens by the declaration within the providers at the app.module and Angular behind the scenes is creating an instance of it whenever it is injected. The mark (class type) declared in the constructor is used as a key (injection token) to find the correct object, like the AuthService class, inside the dependence injection registry. One of the purposes of declaring the class type is to auto complete feature used in Intellisense.

Objects have different instances even if receives the same value for the same object, so they might look the same, but they will be two different instances, so it is the same in the dependency injection registry. 

Just as an exemplification of Global services, it was implemented a toastr service and included a token in app.module, where it was added a provide with useValue that receives a global object declared in the app.module, which tells to Angular dependency injection that if some part of application wants toastr (or any other class created), it has to be asked for using the TOASTR_TOKEN declared in [toastr.service](). The TOASTR_TOKEN initialized with new InjectionToken and its constructor takes in a single type parameter whiche is a string ('toastr'), that has the same name of global object created in app.module, but it is not important what put there other than that it is a string that we can recognize, so that was put the type as the same of the object that is given back for the service. So this token is used to look up the instance of the ToastrService. Despite of TOASTR_TOKEN has been initialized with a string value, it is not a string but a Javascript object, which is important to garantee identity because of not having the same instace (mentioned above).

So created all of above, in the [profile component](src/app/user/profile.component.ts) it is consumed. The first thing is to declare within costructor injecting the service, import was necessary for TOASTR_TOKEN and ToastrService as done in the app.module. Another thing was the Inject import (that is a decorator as @Component), and it allows to use a separate token besides the type of the construction parameter. So the @Inject decorator was used before the constructor variable and tells to Angular that it is been created a member of this class to get a value by using the TOASTR_TOKEN to look up the service in the dependency injection registry. This way now it is just to declare within a method (saveProfile was taken as example)

## Session Search [video](https://app.pluralsight.com/course-player?clipId=6d2f9fed-93a4-4407-91f4-8f7ec09d92a1)

The search bar aims to look for a specific session within an event and it be showed a popped-up list, then click in one of the sessions brought and navigate to the details for that event. A Bootstrap's modal dialog was used, which is acctually added into jQuery, that means it has both jQuery and Bootstrap and Angular wrappers are used around that functionality.

First was to add a submit event to the search bar that is acctually a form ([searchForm](src/app/nav/navbar.component.html)), so ngSubmit allows to the user to hit Enter or click the button and the searchSessions method was created with searchTerm that is whatever the user types into the input box. To hook up the input box was used ngModel, giving a name with the same name used to the variable (searchTerm). A property called foundSessions was added to load ISession array, that will receive data to be searched. To not having the component itself doing the search, a service it is necessary to handle that, so the eventService was chosen for this, it seems like a natural place to put the functionality. A method called searchSessions was created in this service that returns an observable, but it the data is locally, it could just return the data synchronously, however an observable is fairly appropriate for using with http call. In the subscribe it receives back a list of sessions and after that, it will set the foundSessions variable.

The method in the eventService lower case the searchTerm received as parameter just to make it a little bit easier, so no matter what the user types in, it will be consistent. So inside this method, it iterates the array eventList in order to look for the term typed by the user and it is kept in the matchingSessions variable the indexOf of the term in lower case that is greater than -1, and mapping this variable to take the event id and put it a new variable called session.eventId, because it is a quick and easy way to add the eventId to every session that has been created. After that, take the results array and add to it by using concat method the matchingSessions. In order to return it to be used in the html it is necessary to use an observable, and a simple way to do that is using EventEmitter, that passing true will tell this emitter to deliver its events asynchronously. Just to simulation matters a web request, it was put setTimeout() within the searchSessions in the event.service.
